-- Enforce package-to-class eligibility at the booking transaction boundary.
-- UI filtering is helpful, but it cannot be trusted: a direct API request
-- must not redeem a Classic credit against Restore/Ballet, or vice versa.
...
declare
  ...
  v_package_service_slug public.service_slug;
...
begin
  ...
  select p.service_slug
    into v_package_service_slug
    from public.packages as p
   where p.id = v_pkg.package_id;

  select ct.service_slug
    into v_session_service_slug
    from public.class_types as ct
   where ct.id = v_session.class_type_id;

  if v_session_service_slug is null or (
    v_package_service_slug is null
    and v_session_service_slug not in ('mat-pilates', 'yoga', 'barre', 'strength-hiit')
  ) or (
    v_package_service_slug is not null
    and v_package_service_slug <> v_session_service_slug
  ) then
...
revoke execute on function public.book_class_session(uuid, uuid) from public, anon;
