...
- **Domain / email domain.** Veora has no confirmed registered domain yet. The site currently uses `veorawellness.com` as a placeholder (matching the pattern `hello@veorawellness.com`, `NEXT_PUBLIC_SITE_URL`, `RESEND_FROM_EMAIL`) — update every reference once a real domain is secured.
...
- **No distinct "booking email"** was published separately from the general studio email — both currently use `hello@veorawellness.com`. Confirm whether a dedicated booking inbox should be used instead.
